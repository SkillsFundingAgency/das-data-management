...
,('Apply','Contacts','dbo','[Id],[ApplyOrganisationID],[Status],[IsApproved],[CreatedAt],[UpdatedAt],[DeletedAt]','[Email],[GivenNames],[FamilyName],[ContactDetails]'
		...
		,'select [Id],[Email],[GivenNames],[FamilyName],[SigninId],[SigninType],[ApplyOrganisationID],[ContactDetails],[Status],[IsApproved],[CreatedAt],case when TRY_CONVERT(UNIQUEIDENTIFIER,[CreatedBy]) IS NOT NULL Then [CreatedBy] Else  STUFF([CreatedBy],2,len([CreatedBy])-2,REPLICATE(''''*'''',len([CreatedBy])-2)) End As [CreatedBy],[UpdatedAt],[DeletedAt]  from [dbo].[Contacts]','RP_Contacts')
...
(SourceDatabaseName,SourceTableName,SourceSchemaName,ColumnNamesToInclude,ColumnNamesToExclude,ColumnNamesToMask,ModelDataToPL,IsQueryBasedImport,SourceQuery,StagingTableName)
VALUES
   ('EmployerDemand','CourseDemand','dbo','[Id],[ContactEmailAddress],[OrganisationName],[NumberOfApprentices],[CourseId],[CourseTitle],[CourseLevel],[CourseRoute],[LocationName],[Lat],[Long],[DateCreated],[EmailVerified],[DateEmailVerified],[Stopped],[DateStopped],[ExpiredCourseDemandId],[EntryPoint]','','',0,1
      ,'select [Id],[ContactEmailAddress],[OrganisationName],[NumberOfApprentices],[CourseId],[CourseTitle],[CourseLevel],[CourseRoute],[LocationName],[Lat],[Long],[DateCreated],[EmailVerified],[DateEmailVerified],[Stopped],[DateStopped],[ExpiredCourseDemandId],[EntryPoint] FROM (SELECT [Id],STUFF([ContactEmailAddress],2,charindex(''''@'''',[ContactEmailAddress])-3,REPLICATE(''''*'''',charindex(''''@'''',[ContactEmailAddress])-3)) As [ContactEmailAddress],[OrganisationName],[NumberOfApprentices],[CourseId],[CourseTitle],[CourseLevel],[CourseRoute],STUFF([LocationName],2,charindex('''' '''',[LocationName])-3,REPLICATE(''''*'''',charindex('''' '''',[LocationName])-3)) As [LocationName],STUFF([Lat],6,charindex(''''.'''',[Lat])+1,REPLICATE(''''*'''',charindex(''''.'''',[Lat])+1)) As [Lat],STUFF([Long],6,charindex(''''.'''',[Long])+1,REPLICATE(''''*'''',charindex(''''.'''',[Long])+1)) As [Long],[DateCreated],[EmailVerified],[DateEmailVerified],[Stopped],[DateStopped],[ExpiredCourseDemandId],[EntryPoint] FROM [dbo].[CourseDemand]) a '
      ,'AED_CourseDemand')
   ,('EmployerDemand','CourseDemandNotificationAudit','dbo','[Id],[CourseDemandId],[DateCreated],[NotificationType]','','',0,1
      ,'Select [Id],[CourseDemandId],[DateCreated],[NotificationType] FROM (Select [Id],[CourseDemandId],[DateCreated],[NotificationType] From [dbo].[CourseDemandNotificationAudit]) as a '
      ,'AED_CourseDemandNotificationAudit')
   ,('EmployerDemand','ProviderInterest','dbo','[Id],[EmployerDemandId],[Email],[Phone],[Website],[DateCreated]','','[Ukprn]',0,1
    ,'Select [Id],[EmployerDemandId],[Ukprn],[Email],[Phone],[Website],[DateCreated] FROM (Select [Id],[EmployerDemandId],[Ukprn],STUFF([Email],2,charindex(''''@'''',[Email])-3,REPLICATE(''''*'''',charindex(''''@'''',[Email])-3)) As [Email],STUFF([Phone],5,Len([Phone])-2,REPLICATE(''''*'''',Len([Phone])-2)) As [Phone],[Website],[DateCreated]  From [dbo].[ProviderInterest]) As a '
    ,'AED_ProviderInterest')
