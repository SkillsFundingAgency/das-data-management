CREATE PROCEDURE [dbo].[ImportVacanciesApplicationToPL_Rcrt]
(
   @RunId int
)
AS
...
/* Start Logging Execution */
  ...
  SELECT 
        @RunId
	   ,'Step-6'
	   ,'ImportVacanciesApplicationToPL_Rcrt'
	   ,getdate()
	   ,0

  SELECT @LogID=MAX(LogId) FROM Mgmt.Log_Execution_Results
   WHERE StoredProcedureName='ImportVacanciesApplicationToPL_Rcrt'
     AND RunId=@RunID
...
INSERT INTO [ASData_PL].[Va_Application_Rcrt]
           ([CandidateId]
           ,[VacancyId]
