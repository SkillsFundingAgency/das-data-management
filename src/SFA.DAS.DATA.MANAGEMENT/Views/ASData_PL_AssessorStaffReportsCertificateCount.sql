CREATE VIEW [ASData_PL].[AssessorStaffReportsCertificateCount]
As
	SELECT 
		...
		(SELECT COUNT(*) FROM [ASData_PL].[Assessor_CertificateLogs] WHERE [Status] = 'DUPLICATE' AND Username = 'Manual') AS 'Duplicated',
		(SELECT COUNT(DISTINCT ce2.[OrganisationId]) FROM [ASData_PL].[Assessor_Certificates] ce2 WHERE ce2.CreatedBy <> 'Manual' AND ce2.[Type]='Standard') AS 'EPAOs',
