...
,('Commitments','RegDetails','dbo','[Id],[CommitmentId],[ApprenticeshipId],[FirstName],[LastName],[FullName],[EmailAddress]','','','Comt_ApprenticeshipRegDetails','Comt_ApprenticeshipRegDetails',0,1,'SELECT [Id],[CommitmentId],[ApprenticeshipId],convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(VARCHAR(255),firstname)))), @SaltKey)))),2) as firstname,convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(VARCHAR(255),lastname)))), @SaltKey)))),2) as lastname, convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(VARCHAR(255),concat(isnull(ltrim(rtrim(firstname)),''''''''),isnull(ltrim(rtrim(lastname)),'''''''')))))), @SaltKey)))),2) as FullName, convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(VARCHAR(255),Email)))), @SaltKey)))),2) as EmailAddress FROM [dbo].[Apprenticeship]')
...
('Apply','Apply','NA','[ID],[ApplicationId],[OrganisationId],[ReferenceNumber],[ApplicationSubmittedOn],[ProviderRoute],[ProviderRouteName],[ApplicationStatus],[OutcomeDateTime],[GatewayReviewStatus],[AssessorReviewStatus],[ModerationStatus],[ModeratorClarificationRequestedOn],[CreatedAt],[UpdatedAt],[DeletedAt],[Assessor1ReviewStatus],[Assessor2ReviewStatus],[ApplicationDeterminedDate],[Comments],[ExternalComments],[ProviderRouteOnRegister],[ProviderRouteNameOnRegister]','','[UKPRN],[OrganisationName],[TradingName],[ApplicationSubmittedBy],[Assessor1Name],[Assessor2Name],[CreatedBy],[GatewayUserName]',0,1
		...
		,'RP_Apply')
,('Apply','Organisation','NA','[Id],[OrganisationType],[Status],[RoATPApproved],[CreatedAt],[UpdatedAt],[DeletedAt],[OrganisationReferenceType],[VerificationId],[PrimaryVerificationSource],[VerificationId],[PrimaryVerificationSource]','[RoEPAOApproved]'
