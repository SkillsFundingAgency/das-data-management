CREATE VIEW [ASData_PL].[DAS_SpendControlNonLevy_V2]
	...
	(
	 SELECT EventId,ApprenticeshipId,FundingSource,TransactionType,Amount,COALESCE(Cast( Payment.AcademicYear AS varchar) + '-R' + RIGHT( '0' + Cast (Payment.CollectionPeriod AS varchar), 2 ),'NA') as PeriodEnd
      ...
      WHERE ApprenticeshipId in (select id from ASData_PL.Comt_Apprenticeship where ReservationId in (select id from ASData_PL.Resv_Reservation WHERE YEAR(CreatedDate) > = 2020 AND IsLevyAccount = 0	))
	)
