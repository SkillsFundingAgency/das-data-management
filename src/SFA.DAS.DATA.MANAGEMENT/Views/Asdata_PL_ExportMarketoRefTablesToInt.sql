...
SELECT 
    ...
    CASE WHEN ur.Role IS NOT NULL THEN 'Y' ELSE 'N' END AS [Stage1b_UserAccountRole],
    CASE WHEN paye.[PayeAdded] IS NOT NULL THEN 'Y' ELSE 'N' END AS [Stage2_PayeAdded],
    CASE 
        WHEN a.[NameConfirmed] = 0 THEN 'N'
        WHEN a.[NameConfirmed] = 1 AND a.ApprenticeshipEmployerType = 2 THEN 'N'
        WHEN a.[NameConfirmed] = 1 AND a.Name = 'MY ACCOUNT' THEN 'N'
        WHEN a.[NameConfirmed] = 1 THEN 'Y' 
        ELSE 'N' 
    END AS [Stage3_AccountNameConfirmed],
    CASE WHEN ea.id IS NULL THEN 'N' ELSE 'Y' END AS [Stage4_EmployerAgreementSigned],
    CASE
        WHEN a.AddTrainingProviderAcknowledged = 1 THEN 'Y'
        WHEN prov.AccountId IS NULL THEN 'N' 
        ELSE 'Y' 
