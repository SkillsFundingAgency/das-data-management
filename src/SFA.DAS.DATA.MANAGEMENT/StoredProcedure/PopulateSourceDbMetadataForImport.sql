...
(SourceDatabaseName,SourceTableName,SourceSchemaName,ColumnNamesToInclude,ColumnNamesToExclude,ColumnNamesToMask,ModelDataToPL,IsQueryBasedImport,SourceQuery,StagingTableName)
VALUES
 ('Candacc','Candidate','dbo','[Id],[GovUkIdentifier],[FirstName],[MiddleNames],[LastName],[FullName],[Email],[DateOfBirth],[DOB],[CreatedOn],[UpdatedOn],[MigratedEmail],[MigratedCandidateId]','[PhoneNumber],[TermsOfUseAcceptedOn],[Status]','',1,1,'SELECT id,GovUkIdentifier,convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(VARCHAR(255),firstname)))), @SaltKey)))),2) as FirstName, convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(VARCHAR(255),middlenames)))), @SaltKey)))),2) as MiddleNames, convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(VARCHAR(255),LastName)))), @SaltKey)))),2) as Lastname, convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(VARCHAR(255),concat(isnull(ltrim(rtrim(firstname)),''''''''),isnull(ltrim(rtrim(middlenames)),''''''''),isnull(ltrim(rtrim(Lastname)),'''''''')))))), @SaltKey)))),2) as FullName, convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(DATE,dateofbirth)))), @SaltKey)))),2) as DateOfBirth,convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(CONVERT(varchar(255),Email)))), @SaltKey)))),2) as Email,dateofbirth as DOB,[CreatedOn],[UpdatedOn],[MigratedEmail],[MigratedCandidateId] FROM dbo.candidate','FAAV2_Candidate')
...
,('Accounts','Account','Employer_Account','[Id],[HashedId],[CreatedDate],[ModifiedDate],[ApprenticeshipEmployerType],[NameConfirmed],[AddTrainingProviderAcknowledged]','','[Name],[PublicHashedId]','Acc_Account',1)
...
,('Commitments','Standard','dbo','[StandardUId],[LarsCode],[IFateReferenceNumber],[Version],[Level],[Duration],[MaxFunding],[EffectiveFrom],[EffectiveTo],[VersionMajor],[VersionMinor],[StandardPageUrl],[Status],[IsLatestVersion],[VersionEarliestStartDate],[VersionLatestStartDate],[Route],[Title]','','','Comt_Standard',0)
,('Commitments','StandardOption','dbo','[StandardUId],[Option]','','','Comt_StandardOption',0)
,('Commitments','ApprenticeshipPriorLearning','dbo','[ApprenticeshipId],[DurationReducedBy],[PriceReducedBy],[IsAccelerated],[DurationReducedByHours],[IsDurationReducedByRpl]','','','Comt_ApprenticeshipPriorLearning',0)
...
,('Commitments','OverlappingTrainingDateRequest','dbo','[Id],[DraftApprenticeshipId],[PreviousApprenticeshipId],[ResolutionType],[Status],[CreatedOn],[ActionedOn],[NotifiedServiceDeskOn],[NotifiedEmployerOn]','[RequestCreatedByProviderEmail],[RowVersion]','','Comt_OverlappingTrainingDateRequest',0)
,('Commitments','FileUploadCohortLog','dbo','[Id], [FileUploadLogId],[CommitmentId],[RowCount]','','','Comt_FileUploadCohortLog',0)
...
VALUES
 ...
 ,('Assessor','CertificateLogs','dbo','[Id],[Action],[CertificateId],[EventTime],[Status],[BatchNumber],[ReasonForChange],[LatestEpaOutcome],[StandardName],[StandardLevel],[StandardPublicationDate],[ContactOrganisation],[Registration],[LearningStartDate],[AchievementDate],[CourseOption],[OverallGrade],[Department]','','[ContactPostCode],[Username]',1,1,
	'select [Id],[Action],[CertificateId],[EventTime],[Status],[BatchNumber],[ReasonForChange],[LatestEpaOutcome],[StandardName],[StandardLevel],[StandardPublicationDate],[ContactOrganisation],left(ContactPostCode,len(ContactPostCode)-charindex('''' '''',ContactPostCode)+1) [ContactPostCode],[Registration],[LearningStartDate],[AchievementDate],[CourseOption],[OverallGrade],[Department],[Username] FROM ( select [Id],[Action],[CertificateId],[EventTime],[Status],[BatchNumber],[ReasonForChange],[LatestEpaOutcome],JSON_VALUE(CertificateData,''''$.StandardName'''') [StandardName],JSON_VALUE(CertificateData,''''$.StandardLevel'''') [StandardLevel],JSON_VALUE(CertificateData,''''$.StandardPublicationDate'''') [StandardPublicationDate],JSON_VALUE(CertificateData,''''$.ContactOrganisation'''') [ContactOrganisation],JSON_VALUE(CertificateData,''''$.ContactPostCode'''') [ContactPostCode],JSON_VALUE(CertificateData,''''$.Registration'''') [Registration],JSON_VALUE(CertificateData,''''$.LearningStartDate'''') [LearningStartDate],JSON_VALUE(CertificateData,''''$.AchievementDate'''') [AchievementDate],JSON_VALUE(CertificateData,''''$.CourseOption'''') [CourseOption],JSON_VALUE(CertificateData,''''$.OverallGrade'''') [OverallGrade],JSON_VALUE(CertificateData,''''$.Department'''') [Department],[Username] from [dbo].[CertificateLogs] ) As Query',
 ...
 ,('Assessor','OrganisationStandardVersion','dbo','[StandardUId],[Version],[OrganisationStandardId],[EffectiveFrom],[EffectiveTo],[DateVersionApproved],[Comments],[Status]','','',0,1,'Select [StandardUId],[Version],[OrganisationStandardId],[EffectiveFrom],[EffectiveTo],[DateVersionApproved],[Comments],[Status] From [dbo].[OrganisationStandardVersion]','Assessor_OrganisationStandardVersion')
 ,('Assessor','OrganisationType','dbo','[Id],[Type],[Status],[TypeDescription],[FinancialExempt]','','',0,1,'SELECT [Id],[Type],[Status],[TypeDescription],[FinancialExempt]  FROM [dbo].[OrganisationType]','Assessor_OrganisationType')
 ,('Assessor','Standards','dbo','[StandardUId],[Title],[Version],[Level],[Status],[TypicalDuration],[MaxFunding],[IsActive],[LastDateStarts],[EffectiveFrom],[EffectiveTo],[VersionEarliestStartDate],[VersionLatestStartDate],[VersionLatestEndDate],[VersionApprovedForDelivery],[ProposedTypicalDuration],[ProposedMaxFunding]','','[IFateReferenceNumber],[LarsCode]',1,1,
    ...
    'Assessor_Standards')
,('Assessor','Apply','dbo','[Id],[ApplicationId],[OrganisationId],[ApplicationStatus],[ReviewStatus],[LatestStandardSubmissionDate],[StandardSubmissionClosedDate],[FinancialReviewStatus],[FinancialDueDate],[StandardCode],[CreatedAt],[UpdatedAt],[DeletedAt],[StandardApplicationType],[StandardReference]','','[SelectedGrade],[GradedBy],[CreatedBy]',0,1,
    'select [Id],[ApplicationId],[OrganisationId],[ApplicationStatus],[ReviewStatus],[LatestStandardSubmissionDate],[StandardSubmissionClosedDate],[FinancialReviewStatus],[SelectedGrade],[FinancialDueDate],[GradedBy],[StandardCode],[CreatedAt],[CreatedBy],[UpdatedAt],[DeletedAt],[StandardApplicationType],[StandardReference] FROM ( Select [Id],[ApplicationId],[OrganisationId],[ApplicationStatus],[ReviewStatus],JSON_VALUE([ApplyData],''''$.Apply.LatestStandardSubmissionDate'''') LatestStandardSubmissionDate,JSON_VALUE([ApplyData],''''$.Apply.StandardSubmissionClosedDate'''') StandardSubmissionClosedDate,[FinancialReviewStatus],JSON_VALUE([FinancialGrade],''''$.SelectedGrade'''') SelectedGrade,JSON_VALUE([FinancialGrade],''''$.FinancialDueDate'''') FinancialDueDate,JSON_VALUE([FinancialGrade],''''$.GradedBy'''') GradedBy,[StandardCode],[CreatedAt],[CreatedBy],[UpdatedAt],[DeletedAt],[StandardApplicationType],[StandardReference]  from [dbo].[Apply] ) As a ',
    'Assessor_Apply')
,('Assessor','Learner','dbo','[Id],[StdCode],[LearnStartDate],[EpaOrgId],[FundingModel],[ApprenticeshipId],[Source],[CompletionStatus],[PlannedEndDate],[LearnActEndDate],[WithdrawReason],[Outcome],[AchDate],[OutGrade],[Version],[VersionConfirmed],[CourseOption],[StandardUId],[StandardReference],[StandardName],[LastUpdated],[EstimatedEndDate],[ApprovalsStopDate],[ApprovalsPauseDate],[ApprovalsCompletionDate],[ApprovalsPaymentStatus],[LatestIlrs],[LatestApprovals],[Uln],[UkPrn],[IsTransfer],[DelLocPostCode]','[GivenNames],[FamilyName],[LearnRefNumber]','',1,1,
    'SELECT [Id],[StdCode],[LearnStartDate],[EpaOrgId],[FundingModel],[ApprenticeshipId],[Source],[CompletionStatus],[PlannedEndDate],[LearnActEndDate],[WithdrawReason],[Outcome],[AchDate],[OutGrade],[Version],[VersionConfirmed],[CourseOption],[StandardUId],[StandardReference],[StandardName],[LastUpdated],[EstimatedEndDate],[ApprovalsStopDate],[ApprovalsPauseDate],[ApprovalsCompletionDate],[ApprovalsPaymentStatus],[LatestIlrs],[LatestApprovals],[Uln],[UkPrn],[GivenNames],[FamilyName],[LearnRefNumber],[IsTransfer],[DelLocPostCode] FROM [dbo].[Learner]',
    'Assessor_Learner')
...
(SourceDatabaseName,SourceTableName,SourceSchemaName,ColumnNamesToInclude,ColumnNamesToExclude,ColumnNamesToMask,PLTableName,[ModelDataToPL],[FullCopyToPL])
VALUES
 ('Appfb','ApprenticeFeedbackTarget','dbo','[Id],[ApprenticeId],[ApprenticeshipId],[Status],[ProviderName],[LarsCode],[StandardName],[FeedbackEligibility],[EligibilityCalculationDate],[CreatedOn],[UpdatedOn],[Withdrawn],[IsTransfer],[DateTransferIdentified],[ApprenticeshipStatus]','[StartDate],[EndDate],[StandardUId]','[Ukprn]','Appfb_ApprenticeFeedbackTarget',0,1)
...
(SourceDatabaseName,SourceTableName,SourceSchemaName,ColumnNamesToInclude,ColumnNamesToExclude,ColumnNamesToMask,PLTableName,[ModelDataToPL],[FullCopyToPL])
VALUES
 ('evs','CheckType','dbo','[CheckTypeId],[CheckType]','','','EVS_CheckType',0,1)
,('evs','ApprovalsStatus','dbo','[ApprovalsStatusId], [Status]','','','EVS_ApprovalsStatus',0,1)
,('evs','CommitmentStatus','dbo','[CommitmentStatusId], [CommitmentStatus]','','','EVS_CommitmentStatus',0,1)
,('evs','PaymentStatus','dbo','[PaymentStatusId], [PaymentStatus]','','','EVS_PaymentStatus',0,1)
...
VALUES
	...
	,('evs','EmploymentVerification','dbo','[EmploymentVerificationId], [CorrelationId], [ApprenticeshipId], [Employed], [EmploymentCheckDate], [EmploymentCheckRequestDate], [RequestCompletionStatus], [ErrorType], [MessageSentDate], [MinDate], [MaxDate], [CheckTypeId], [CreatedOn], [LastUpdatedOn]','','',0,1
	   ,'select [EmploymentVerificationId], [CorrelationId], [ApprenticeshipId], [Employed], [EmploymentCheckDate], [EmploymentCheckRequestDate], [RequestCompletionStatus], [ErrorType], [MessageSentDate], [MinDate], [MaxDate], [CheckTypeId], [CreatedOn], [LastUpdatedOn] FROM [dbo].[EmploymentVerification]'
...
(SourceDatabaseName,SourceTableName,SourceSchemaName,ColumnNamesToInclude,ColumnNamesToExclude,ColumnNamesToMask,ModelDataToPL,IsQueryBasedImport,SourceQuery,StagingTableName)
VALUES
   	 ('Digc','ActionType','dbo','[Id],[Description]','','',1,1,'SELECT [Id],[Description] FROM [dbo].[ActionType]','Digc_ActionType')
    ,('Digc','AdminActions','dbo','[Id],[ActionTime],[Action],[UserActionId]','','[Username]',1,1,'SSELECT [Id],[Username],[ActionTime],[Action],[UserActionId] FROM [dbo].[AdminActions]','Digc_AdminActions')
    ...
    ,('Digc','SharingAccess','dbo','[Id],[SharingId],[AccessedAt] FROM [dbo].[SharingAccess]','','',1,1,'SELECT  [Id],[SharingId],[AccessedAt] FROM [dbo].[SharingAccess]','Digc_SharingAccess')
	,('Digc','SharingEmail','dbo','[Id],[SharingId],[EmailLinkCode],[SentTime]','','[EmailAddress]',1,1,'SELECT  [Id],[SharingId],[EmailAddress],[EmailLinkCode],[SentTime] FROM [dbo].[SharingEmail]','Digc_SharingEmail')
	,('Digc','SharingEmailAccess','dbo','[Id],[SharingEmailId],[AccessedAt]','','',1,1,'SELECT  [Id],[SharingEmailId],[AccessedAt] FROM [dbo].[SharingEmailAccess]','Digc_SharingEmailAccess')
	,('Digc','User','dbo','[Id]  ,[LastLoginAt] ,[IsLocked] ,[CreatedAt]','','[GovUkIdentifier] ,[EmailAddress] ,[PhoneNumber]',1,1,'SELECT   [Id] ,[GovUkIdentifier] ,[EmailAddress] ,[PhoneNumber] ,[LastLoginAt] ,[IsLocked] ,[CreatedAt] FROM [dbo].[User]','Digc_User')
	,('Digc','UserActions','dbo','[Id],[UserId],[ActionType],[ActionCode],[ActionTime],[CertificateId],[CertificateType],[CourseName]','','[FamilyName],[GivenNames]',1,1,'SELECT  [Id],[UserId],[ActionType],[ActionCode],[ActionTime],[FamilyName],[GivenNames],[CertificateId],[CertificateType],[CourseName] FROM [dbo].[UserActions]','Digc_UserActions')
	,('Digc','UserAuthorisation','dbo','[Id],[Description]','','',1,1,'SELECT  [Id],[UserId],[ULN],[AuthorisedAt] FROM [dbo].[UserAuthorisation]','Digc_UserAuthorisation')
	,('Digc','UserIdentity]','dbo','[Id],[UserId],[ValidSince],[ValidUntil]','','[FamilyName],[DateOfBirth],[GivenNames]',1,1,'SELECT  [Id],[UserId],[FamilyName],[DateOfBirth],[GivenNames],[ValidSince],[ValidUntil] FROM [dbo].[UserIdentity]','Digc_UserIdentity]')
	,('Digc','UserMatch','dbo','[Id],[UserId],[Uln],[FamilyName],[DateOfBirth],[EventTime],[CertificateType],[CourseCode],[CourseName],[CourseLevel],[YearAwarded],[ProviderName],[Ukprn],[IsMatched],[IsFailed]','','[FamilyName],[DateOfBirth]',1,1,'SELECT [Id],[UserId],[Uln],[FamilyName],[DateOfBirth],[EventTime],[CertificateType],[CourseCode],[CourseName],[CourseLevel],[YearAwarded],[ProviderName],[Ukprn],[IsMatched],[IsFailed] FROM [dbo].[UserMatch]','Digc_UserMatch')
