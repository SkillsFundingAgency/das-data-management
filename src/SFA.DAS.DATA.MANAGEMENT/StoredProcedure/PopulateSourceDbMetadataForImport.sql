...
VALUES
 ...
 ,('AvmsPlus','CandidateDetails','NA','[CandidateId],[CandidateGuid],[FirstName],[SurName],[DateOfBirth]','','',1,1,'SELECT c.candidateid, c.candidateguid, convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(p.firstname))), @SaltKey)))),2), convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(p.surname))), @SaltKey)))),2), convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),LTRIM(RTRIM(c.dateofbirth))), @SaltKey)))),2) FROM dbo.candidate c LEFT JOIN dbo.person p  ON c.personid=p.personid','Avms_CandidatePersonalInfo')
