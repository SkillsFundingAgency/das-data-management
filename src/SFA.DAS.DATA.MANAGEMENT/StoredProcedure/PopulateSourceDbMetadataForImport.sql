...
VALUES
   ('Apply','Apply','NA','[ID],[ApplicationId],[OrganisationId],[ReferenceNumber],[ApplicationSubmittedOn],[ApplicationSubmittedBy],[ProviderRoute],[ProviderRouteName],[ApplicationStatus],[OutcomeDateTime],[GatewayReviewStatus],[AssessorReviewStatus],[ModerationStatus],[ModeratorClarificationRequestedOn],[Assessor1Name],[Assessor2Name],[FinancialReviewStatus],[FinancialClarificationRequestedOn],[CreatedAt],[UpdatedAt],[DeletedAt]','','[UKPRN],[OrganisationName],[TradingName],[ApplicationSubmittedBy],[Assessor1Name],[Assessor2Name],[FinancialClarificationRequestedBy],[AssessorName]',0,1
      ,'SELECT Id,ApplicationId,OrganisationId,UKPRN,JSON_VALUE(ApplyData,''''$.ApplyDetails.ReferenceNumber'''') ReferenceNumber,JSON_VALUE(ApplyData,''''$.ApplyDetails.OrganisationName'''') OrganisationName,JSON_VALUE(ApplyData,''''$.ApplyDetails.TradingName'''') TradingName,JSON_VALUE(ApplyData,''''$.ApplyDetails.ProviderRoute'''') ProviderRoute  ,JSON_VALUE(ApplyData,''''$.ApplyDetails.ApplicationSubmittedBy'''') ApplicationSubmittedBy,JSON_VALUE(ApplyData,''''$.ApplyDetails.ApplicationSubmittedOn'''') ApplicattionSubmittedOn,JSON_VALUE(ApplyData,''''$.ApplyDetails.ProviderRouteName'''') ProviderRouteName  ,ApplicationStatus,JSON_VALUE(ApplyData,''''$.GatewayReviewDetails.OutcomeDateTime'''') OutcomeDateTime,GatewayReviewStatus,AssessorReviewStatus,ModerationStatus,JSON_VALUE(ApplyData,''''$.ModeratorReviewDetails.ClarificationRequestedOn'''') ModeratorClarificationRequestedOn,Assessor1Name,Assessor2Name,FinancialReviewStatus,JSON_VALUE(ApplyData,''''$.FinancialReviewDetails.ClarificationRequestedBy'''') FinancialClarificationRequestedBy,JSON_VALUE(ApplyData,''''$.FinancialReviewDetails.ClarificationRequestedOn'''') FinancialClarificationRequestedOn,JSON_VALUE(ApplyData,''''$.FinancialReviewDetails.GradedBy'''') AssessorName ,CreatedAt,CreatedBy,UpdatedAt,UpdatedBy,DeletedAt,DeletedBy from dbo.Apply'
      ,'RP_Apply')
