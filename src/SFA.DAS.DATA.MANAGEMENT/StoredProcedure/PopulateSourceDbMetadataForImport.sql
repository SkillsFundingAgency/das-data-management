...
,('Apply','Organisation','NA','[Id],[OrganisationType],[Status],[RoEPAOApproved],[RoATPApproved],[CreatedAt],[UpdatedAt],[DeletedAt],[OrganisationReferenceType],[VerificationId],[PrimaryVerificationSource],[VerificationId],[PrimaryVerificationSource]',''
		...
		,'RP_Organisations')
,('Apply','Appeal','dbo','[Id],[ApplicationId],[Status],[HowFailedOnPolicyOrProcesses],[HowFailedOnEvidenceSubmitted],[AppealSubmittedDate],[AppealDeterminedDate],[InternalComments],[ExternalComments],[InProgressDate],[InProgressInternalComments],[InProgressExternalComments],[CreatedOn],[UpdatedOn]',''
		,'[UserId],[UserName],[InProgressUserId],[InProgressUserName]',0,1
		,'select [Id],[ApplicationId],[Status],[HowFailedOnPolicyOrProcesses],[HowFailedOnEvidenceSubmitted],[AppealSubmittedDate],[AppealDeterminedDate],[InternalComments],[ExternalComments],STUFF([UserId],2,len([UserId])-2,REPLICATE(''''*'''',len([UserId])-2)) As [UserId],STUFF([UserName],2,len([UserName])-2,REPLICATE(''''*'''',len([UserName])-2)) As [UserName],[InProgressDate],STUFF([InProgressUserId],2,len([InProgressUserId])-2,REPLICATE(''''*'''',len([InProgressUserId])-2)) As [InProgressUserId],STUFF([InProgressUserName],2,len([InProgressUserName])-2,REPLICATE(''''*'''',len([InProgressUserName])-2)) As [InProgressUserName],[InProgressInternalComments],[InProgressExternalComments],[CreatedOn],[UpdatedOn]   From [dbo].[Appeal]')
