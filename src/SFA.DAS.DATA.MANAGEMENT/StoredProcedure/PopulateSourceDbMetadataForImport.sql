...
,('Apply','SubmittedApplicationAnswers','dbo','[Id],[ApplicationId],[PageId],[Answer]',''
		...
		,'select [Id],[ApplicationId],[PageId],[QuestionId],[QuestionType],Case when UPPER(TRIM([ColumnHeading])) IN(''''EMAIL'''',''''CONTACT NUMBER'''',''''FIRST NAME'''',''''LAST NAME'''',''''NAME'''',''''DATE OF BIRTH'''') Then ''''XXXXXXXXXX'''' when Answer LIKE ''''0[1-9][0-9][0-9][0-9][0-9][0-9][0-9]%'''' Then ''''XXXXXXXXXX'''' when trim(UPPER(Answer)) LIKE ''''%@%'''' AND  (trim(UPPER(Answer)) LIKE ''''%.COM%'''' OR trim(UPPER(Answer)) LIKE ''''%.CO.UK%'''') Then ''''XXXXXXXXXX'''' when trim(UPPER(Answer)) LIKE ''''+44%'''' Then ''''XXXXXXXXXX'''' when trim(UPPER(QuestionType)) =''''NUMBER''''  AND len(trim(Answer)) > 7 Then ''''XXXXXXXXXX'''' when trim(UPPER(QuestionType)) =''''EMAIL'''' Then ''''XXXXXXXXXX'''' when trim(UPPER(Answer)) like ''''[A-Z][0-9][0-9]%''''  OR trim(UPPER(Answer)) like ''''[A-Z][A-Z][0-9][0-9]%'''' OR trim(UPPER(Answer)) like ''''[A-Z][A-Z][0-9]%'''' Then ''''XXXXXXXXXX'''' when len(answer) - len(replace(answer,'''' '''', '''''''')) = 1 AND trim(UPPER(Answer)) like ''''[A-Z][A-Z]%'''' Then ''''XXXXXXXXXX'''' Else Answer End Answer,[ColumnHeading] from [dbo].[SubmittedApplicationAnswers]'
