CREATE VIEW [AsData_PL].[ExportMarketoRefTablesToInt_Inc]
As
WITH CTE_EmployerAgreement AS (
 ...
 FROM 
    ...
    JOIN 
		...
		and (cast (a.createddate as date)= DATEADD(day, -1, cast(GETDATE() as date))  or cast (a.ModifiedDate as date)= DATEADD(day, -1, cast(GETDATE() as date)))
    JOIN 
        [asdata_pl].[acc_employeragreement] ea ON ea.accountlegalentityid = ale.id 
		and cast (ale.created as date)= DATEADD(day, -1, cast(GETDATE() as date))
    WHERE ale.signedagreementid IS NOT NULL OR (ea.acknowledged = 1 AND ale.signedagreementid IS NULL)	
),
CTE_ProviderAdded AS (
	...
	Where cast (p.created as date)= DATEADD(day, -1, cast(GETDATE() as date))  
    GROUP BY 
    [accountid]
),
CTE_PayeAdded AS (
    ...
    FROM 
        [ASData_PL].[Acc_AccountHistory] where  1=1
		and	(cast (AddedDate as date)= DATEADD(day, -1, cast(GETDATE() as date)) or cast (Removeddate as date)= DATEADD(day, -1, cast(GETDATE() as date)) )
    GROUP BY 
        [accountid]
),
...
WHERE (ea.id is not null or prov.AccountId is not null  or paye.AccountId is not null)
and ur.Role IN (1, 2) 
    AND prb.id IS NULL 
