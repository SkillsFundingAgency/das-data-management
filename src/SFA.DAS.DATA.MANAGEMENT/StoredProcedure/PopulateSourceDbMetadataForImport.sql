...
VALUES
  ('AvmsPlus','ModelledCandidateAgePostcode','NA','[CandidateId]','','[PostCode]',1,1,'SELECT C.CandidateId as [CandidateId],CASE WHEN [c].[DateOfBirth] IS NULL	THEN - 1 WHEN DATEPART([M], [c].[DateOfBirth]) > DATEPART([M], CH.EventDate) OR DATEPART([M], [c].[DateOfBirth]) = DATEPART([M], CH.EventDate) AND DATEPART([DD], [c].[DateOfBirth]) > DATEPART([DD], CH.EventDate) THEN DATEDIFF(YEAR, [c].[DateOfBirth], CH.EventDate) - 1 ELSE DATEDIFF(YEAR, [c].[DateOfBirth], CH.EventDate) END as AgeAtRegistration,PostCode,CASE WHEN CHARINDEX('' '',Postcode)<>0 THEN SUBSTRING(PostCode,1,CHARINDEX('' '',Postcode)) ELSE SUBSTRING(Postcode,1,LEN(Postcode)-3) end as [PostCode] FROM dbo.Candidate c LEFT JOIN dbo.CandidateHistory CH ON CH.CandidateId=C.CandidateId AND CH.Comment=''NAS Exemplar registered Candidate.''','Avms_CandidateAgePostCode')
 ,('AvmsPlus','ModelledAgeAtRegistration','NA','[ApplicationId]','','[AgeAtRegistration]',1,1,'SELECT A.ApplicationId as [ApplicationId],CASE WHEN [c].[DateOfBirth] IS NULL	THEN - 1 WHEN DATEPART([M], [c].[DateOfBirth]) > DATEPART([M], AH.HistoryDate) OR DATEPART([M], [c].[DateOfBirth]) = DATEPART([M], AH.HistoryDate) AND DATEPART([DD], [c].[DateOfBirth]) > DATEPART([DD], AH.HistoryDate) THEN DATEDIFF(YEAR, [c].[DateOfBirth], AH.HistoryDate) - 1 ELSE DATEDIFF(YEAR, [c].[DateOfBirth], AH.HistoryDate) END as [AgeAtRegistration] from dbo.Application A left join (select ApplicationId,min(ApplicationHistoryEventDate) HistoryDate from dbo.ApplicationHistory Ah group by ApplicationId) AH ON AH.ApplicationId=A.ApplicationId left join dbo.Candidate c on c.CandidateId=a.CandidateId','Avms_AgeAtApplication')
/* Employer Incentives */
