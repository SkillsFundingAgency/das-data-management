...
(SourceDatabaseName,SourceTableName,SourceSchemaName,ColumnNamesToInclude,ColumnNamesToExclude,ColumnNamesToMask,ModelDataToPL,IsQueryBasedImport,SourceQuery,StagingTableName)
VALUES
 ('Candacc','Candidate','dbo','[Id],[FirstName],[MiddleNames],[LastName],[Email],[DateOfBirth],[CreatedOn],[UpdatedOn],[GovUkIdentifier],
 [MigratedEmail], [MigratedCandidateId]','[PhoneNumber],[TermsOfUseAcceptedOn], [Status]','',1,1,
 'SELECT id, GovUkIdentifier, convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(CONVERT(NVARCHAR(512),
 LTRIM(RTRIM(CONVERT(VARCHAR(255),firstname)))), @SaltKey)))),2) as FirstName,convert(NVarchar(500),
