CREATE VIEW [ASData_PL].[AssessorStaffReportsByProvider]
As
  ...
  FROM 
	(
		SELECT a1.*,
		(SELECT MAX(UPPER(ProviderName)) FROM [ASData_PL].[Assessor_Certificates] WHERE [ProviderUkPrn] = a1.[ProviderUkPrn]) [ProviderOrganisationName]
