	...
	DECLARE @LogID int
	  ...
	  SELECT @LogID=MAX(LogId) FROM Mgmt.Log_Execution_Results
	   WHERE StoredProcedureName='CreateDashboardReservationAndTrainingView'
		...
		(
			SELECT	B1,B2,B4,B5,B11,B13,B16,B17,B26,CommDateTransfer,CommDateBase,
		...
		(	
			...
			from 
			(
					SELECT	DISTINCT App.Id As C1,Pay.CollectionPeriod AS C2 FROM Comt.Ext_Tbl_Apprenticeship App JOIN StgPmts.Payment Pay ON Pay.LearnerUln = App.Uln	WHERE		Pay.AcademicYear = ''1920''
		...
		Select  
				CASE 
				WHEN r.B1 IS NOT NULL THEN CONVERT(NVARCHAR(500),HASHBYTES(''SHA2_512'',LTRIM(RTRIM(CONCAT(r.B1, SaltKeyData.SaltKey)))),2) 
				END AS B1,				
				r.B13,
				r.B2,				
				CASE 
				WHEN r.B4 IS NOT NULL THEN CONVERT(NVARCHAR(500),HASHBYTES(''SHA2_512'',LTRIM(RTRIM(CONCAT(r.B4, SaltKeyData.SaltKey)))),2) 
				END AS B4,
				r.B5,
