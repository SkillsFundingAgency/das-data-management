...
(SourceDatabaseName,SourceTableName,SourceSchemaName,ColumnNamesToInclude,ColumnNamesToExclude,ColumnNamesToMask,StagingTableName,PLTableName,[ModelDataToPL],[IsQueryBasedImport],SourceQuery)
VALUES
('TPR','Organisation','dbo','[TPRUniqueId],[CompaniesHouseNumber],[TradeClassDescription],[AORN],[EmpRef],[SchemeStartDate],[SchemeEndDate],[SchemeEndDateCodeDesc],[RestartDate],[LiveEmployeeCount],[EmployeeCountDateTaken]','','[OrganisationName],[TradeClass],[PostCode]','TPR_OrgDetails','TPR_OrgDetails',0,1,'SELECT TOrg.TPRUniqueId,TOrg.OrganisationName,convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(Torg.CompanyRegNo, saltkeydata.SaltKey)))),2) as CompaniesHouseNumber,TOrg.TradeClass,CASE WHEN left(TOrg.TradeClass,1)=''''S'''' THEN ''''Society Registered'''' WHEN left(TOrg.TradeClass,1)=''''C'''' THEN ''''Limited Company'''' WHEN left(TOrg.TradeClass,1)=''''I'''' THEN ''''Individual Or Sole Trader'''' WHEN left(TOrg.TradeClass,1)=''''P'''' THEN ''''Public Corporation'''' WHEN left(TOrg.TradeClass,1)=''''F'''' THEN ''''Partnership'''' WHEN left(TOrg.TradeClass,1)=''''L'''' THEN ''''Local Authority'''' WHEN left(TOrg.TradeClass,1)=''''E'''' THEN ''''Close Investment Companies'''' ELSE ''''Unknown'''' END as TradeClassDescription,convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(TOrg.AORN, saltkeydata.SaltKey)))),2) [Aorn],CASE WHEN CHARINDEX('''' '''',toa.Postcode)<>0 THEN SUBSTRING(TOA.PostCode,1,CHARINDEX('''' '''',Postcode)) ELSE SUBSTRING(TOA.Postcode,1,LEN(TOA.Postcode)-3) end as [PostCode],convert(NVarchar(500),HASHBYTES(''''SHA2_512'''',LTRIM(RTRIM(CONCAT(Tops.PayeScheme, saltkeydata.SaltKey)))),2) [EmpRef],TOPS.SchemeStartDate,TOPS.SchemeEndDate,TOPS.SchemeEndDateCodeDesc,TOPS.RestartDate,TOPS.LiveEmployeeCount,TOPS.EmployeeCountDateTaken FROM Tpr.Organisation TOrg LEFT JOIN Tpr.OrganisationAddress TOA ON Torg.TPRUniqueId=TOA.TprUniqueId LEFT JOIN Tpr.OrganisationPAYEScheme TOPS ON TOPS.TPRUniqueId=TOrg.TPRUniqueId CROSS JOIN (Select TOP 1 SaltKeyID,SaltKey From Mgmt.SaltKeyLog Where SourceType =''''EmployerReference''''  Order by SaltKeyID DESC ) SaltKeyData')
