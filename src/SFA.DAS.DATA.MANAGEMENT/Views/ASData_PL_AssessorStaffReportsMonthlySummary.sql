CREATE VIEW [ASData_PL].[AssessorStaffReportsMonthlySummary]
As
	...
	(
		...
		(
			...
			FROM 
				[ASData_PL].Assessor_CertificateLogs
			WHERE 
	...
	[FirstSubmitOrPrintedPassEvent] JOIN [ASData_PL].Assessor_Certificates as [StandardCertificates]
		ON [StandardCertificates].[Id] = [FirstSubmitOrPrintedPassEvent].[CertificateId]
        WHERE [StandardCertificates].[Type]='Standard'
	GROUP BY 
