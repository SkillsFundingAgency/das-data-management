...
,EditStatusDesc  as (Case when EditStatus=0 then 'Both' when EditStatus=1 then 'Employer' when EditStatus=2 then 'Provider' when EditStatus=3 then 'Neither' else 'Unknown' end)
,CommitmentCreatedOn datetime
,LastAction smallint not null
,LastActionDesc as (Case when LastAction=0 then 'New Cohort' when LastAction=1 then 'Amended by a Party' when LastAction=2 then 'Approved by a Party' when lastaction=3 then 'Amended by Employer Following Transfer Sender Rejection' else ' Unknown' end)
