...
BEGIN TRY
		...
		BEGIN TRANSACTION				
				...
				[Hits_IsExit],[EmployerId],[ID2],[ID3],replace(upper([ESFAToken]),'P','') As [ESFAToken],[EventCategory],[EventAction],replace(upper([EventLabel_ESFAToken]),'P','') As [EventLabel_ESFAToken],
				[EventLabel_Keyword],[EventLabel_Postcode],[EventLabel_WithinDistance],[EventLabel_Level],
				[CD_ClientId],[CD_SearchTerms],[CD_UserId],[CD_LevyFlag],[CD_EmployerId],replace(upper([CD_ESFAToken]),'P','') As [CD_ESFAToken],[CD_LegalEntityId],getdate()
