...
INSERT INTO Mtd.SourceConfigForImport
(SourceDatabaseName,SourceTableName,SourceSchemaName,ColumnNamesToInclude,ColumnNamesToExclude,ColumnNamesToMask,PLTableName,[ModelDataToPL],[IsQueryBasedImport],SourceQuery)
VALUES
('Commitments','History','dbo','[Id],[EntityType],[EntityId],[CommitmentId],[ApprenticeshipId],[UserId],[UpdatedByRole],[ChangeType],[CreatedOn],[ProviderId],[EmployerAccountId],[OriginalState_PaymentStatus],[UpdatedState_PaymentStatus],[CorrelationId]','[UpdatedByName],[Diff],','','Comt_History',0,1,'SELECT [Id],[EntityType],[EntityId],[CommitmentId],[ApprenticeshipId],[UserId],[UpdatedByRole],[ChangeType],[CreatedOn],[ProviderId],[EmployerAccountId],[UpdatedByName],CASE WHEN JSON_VALUE(History.OriginalState, ''$.PaymentStatus'') = 1 AND JSON_VALUE(History.UpdatedState, ''$.PaymentStatus'') = 2 THEN ''Active to Pause'' WHEN JSON_VALUE(History.OriginalState, ''$.PaymentStatus'') = 2 AND JSON_VALUE(History.UpdatedState, ''$.PaymentStatus'') = 1 THEN ''Pause to Active'' WHEN JSON_VALUE(History.OriginalState, ''$.PaymentStatus'') = 1 AND JSON_VALUE(History.UpdatedState, ''$.PaymentStatus'') = 3 THEN ''Active to Stop'' WHEN JSON_VALUE(History.OriginalState, ''$.PaymentStatus'') = 2 AND JSON_VALUE(History.UpdatedState, ''$.PaymentStatus'') = 3 THEN ''Pause to Stop'' END UpdatedState_PaymentStatus,CASE WHEN JSON_VALUE(History.OriginalState, ''$.PaymentStatus'') = 1 AND JSON_VALUE(History.UpdatedState, ''$.PaymentStatus'') = 2 THEN ''Paused'' WHEN JSON_VALUE(History.OriginalState, ''$.PaymentStatus'') = 2 AND JSON_VALUE(History.UpdatedState, ''$.PaymentStatus'') = 1 THEN ''Resumed'' WHEN JSON_VALUE(History.OriginalState, ''$.PaymentStatus'') = 1 AND JSON_VALUE(History.UpdatedState, ''$.PaymentStatus'') = 3 THEN ''Stopped'' WHEN JSON_VALUE(History.OriginalState, ''$.PaymentStatus'') = 2 AND JSON_VALUE(History.UpdatedState, ''$.PaymentStatus'') = 3 THEN ''Stopped'' END OriginalState_PaymentStatus,[Diff],[CorrelationId] FROM [dbo].[History]')
